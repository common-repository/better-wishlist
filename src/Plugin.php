...
class Plugin extends Singleton {
	...
	public function add_admin_notice() {
		...
		if ( isset( $installed_plugins[ $basename ] ) ) {
			...
			$message = sprintf( __( '%1$sBetterWishlist%2$s requires %1$sWooCommerce%2$s plugin to be active. Please activate WooCommerce to continue.', 'better-wishlist' ), '<strong>', '</strong>' );
			$button  = '<p><a href="' . esc_url( $activation_url ) . '" class="button-primary">' . __( 'Activate WooCommerce', 'better-wishlist' ) . '</a></p>';
		} else {
			...
			/* translators: %1$s: search term %2%s: search term */
			$message = sprintf( __( '%1$sBetterWishlist%2$s requires %1$sWooCommerce%2$s plugin to be installed and activated. Please install WooCommerce to continue.', 'better-wishlist' ), '<strong>', '</strong>' );
			$button  = '<p><a href="' . esc_url( $activation_url ) . '" class="button-primary">' . __( 'Install WooCommerce', 'better-wishlist' ) . '</a></p>';
		}
